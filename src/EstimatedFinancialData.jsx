...
import './EstimatedFinancialData.css'
  ...
  return (
    <section className="estimated-financial-data">
      ...
      <table className="finTable">
        <tbody>
          <tr>
            <td className="column column1 column-narrow">
              {data.slice(0, 5).map((item, index) => (
                <div key={index}>{extractTextUntilYear(item).label}</div>
              ))}
            </td>
            <td className="column column2 column-narrow">
              {data.slice(0, 5).map((item, index) => (
                <div key={index}>{extractTextUntilYear(item).dataCells}</div>
              ))}
            </td>
            <td className="column column3 column-wide">
              {data.slice(5, 10).map((item, index) => (
                <div key={index}>{extractTextUntilYear(item).label}</div>
              ))}
            </td>
            <td className="column column4 column-wide">
              {data.slice(5, 10).map((item, index) => (
                <div key={index}>{extractTextUntilYear(item).dataCells}</div>
              ))}
            </td>

            <td className="column column5 column-narrow">
              <div>{cap2.label}</div>
